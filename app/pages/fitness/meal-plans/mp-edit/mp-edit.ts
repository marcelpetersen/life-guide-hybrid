...
export class MpEditPage implements OnInit {
    ...
    public createMp(): void {
        ...
        } else if (!this.mealPlan.breakfast.meals.length
            && !this.mealPlan.brunch.meals.length
            && !this.mealPlan.lunch.meals.length
            && !this.mealPlan.snack.meals.length
            && !this.mealPlan.dinner.meals.length
        ) {
            const toast = Toast.create({
